...
RSpec.describe "Categories", type: :request do
  ...
  it 'POST /categories' do
    sign_in User.create!(name: 'Goodman', email: 'bogdan@example.com', password: 'password', confirmed_at: Time.now)
    post('/categories', params: { category: { name: 'Category #n', icon: ActionController::Base.helpers.image_path('education.png') } })
  ...
  it 'PUT /categories/:id' do
    ...
    category = Category.create!(user:, name: 'Category #1', icon: ActionController::Base.helpers.image_path('education.png'))
    sign_in user
    put("/categories/#{category.id}", params: { category: { name: 'Category num.1', icon: ActionController::Base.helpers.image_path('other.png') } })
    category.reload
